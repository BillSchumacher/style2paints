...
def make_graph():
    ...
    feed.extend(
        tf.reduce_mean(features[_], axis=[1, 2]) for _ in range(len(features))
    )
...
def ini_hint(x):
    return np.zeros(shape=(x.shape[0], x.shape[1], 4), dtype=np.uint8)
...
def upload_sketch():
    ...
    print(f'New room ID: {ID}')
    room_path = f'./game/rooms/{ID}/'
    os.makedirs(room_path, exist_ok=True)
    cv2_imwrite(f'{room_path}origin.png', origin)
    ...
    cv2_imwrite(f'{room_path}sketch.png', sketch)
    return f'{ID}_{ID}'
...
def request_result():
    ...
    with open(f'{room_path}/points.{ID}.txt', 'wt') as fp:
        fp.write(points)
    points = json.loads(points)
    for _ in range(len(points)):
        points[_][1] = 1 - points[_][1]
    sketch = cv2.imread(f'{room_path}sketch.png', cv2.IMREAD_UNCHANGED)
    origin = cv2.imread(f'{room_path}origin.png', cv2.IMREAD_GRAYSCALE)
    ...
    smoothed_careless, flat_careless, blended_smoothed_careless, blended_flat_careless = refine_image(careless, sketch,
                                                                                                      origin)
    cv2_imwrite(f'{room_path}/{ID}.smoothed_careless.png', smoothed_careless)
    cv2_imwrite(f'{room_path}/{ID}.flat_careless.png', flat_careless)
    cv2_imwrite(
        f'{room_path}/{ID}.blended_smoothed_careless.png',
        blended_smoothed_careless,
    )
    cv2_imwrite(
        f'{room_path}/{ID}.blended_flat_careless.png', blended_flat_careless
    )
    ...
    smoothed_careful, flat_careful, blended_smoothed_careful, blended_flat_careful = refine_image(careful, sketch,
                                                                                                  origin)
    cv2_imwrite(f'{room_path}/{ID}.smoothed_careful.png', smoothed_careful)
    cv2_imwrite(f'{room_path}/{ID}.flat_careful.png', flat_careful)
    cv2_imwrite(
        f'{room_path}/{ID}.blended_smoothed_careful.png',
        blended_smoothed_careful,
    )
    cv2_imwrite(f'{room_path}/{ID}.blended_flat_careful.png', blended_flat_careful)
